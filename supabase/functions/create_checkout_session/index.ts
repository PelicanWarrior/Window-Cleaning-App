...
serve(async (req) => {
  ...
  try {
    ...
    let user = null as { id: unknown; email_address: string | null; StripeCustomerId: string | null } | null;
    ...
    // Create or get the price for this product
    const priceParams = new URLSearchParams();
    priceParams.append("unit_amount", String(Math.round(monthlyAmount * 100)));
    ...
    const successUrl = `${origin}?checkout=success&session_id={CHECKOUT_SESSION_ID}&user_id=${userId}`;
    ...
    sessionParams.append("metadata[user_id]", String(userId));
    sessionParams.append("metadata[account_level_id]", String(level.id));
    sessionParams.append("subscription_data[metadata][user_id]", String(userId));
