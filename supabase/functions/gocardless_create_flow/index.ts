...
function buildInvoiceDescription(customer: Record<string, any>, invoice: Record<string, any>) {
...
serve(async (req) => {
  ...
  try {
    ...
    if (invoiceId) {
      ...
      const description = buildInvoiceDescription(customer, invoice);
    ...
    return jsonResponse(200, {
      ...
      mode: invoiceId ? (openBankingOnly ? "invoice_open_banking" : "invoice_payment_and_mandate") : "mandate_setup",
    });
  } catch (error) {
