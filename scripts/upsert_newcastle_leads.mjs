...
const leadsPath = path.join(root, 'Letters', 'prospecting', 'newcastle_day1_leads_100.json')
...
function nextNclIdFactory(existingLeadIds) {
  const used = new Set(existingLeadIds.map((id) => String(id || '').trim()).filter(Boolean))
  let n = 1
  return function nextId() {
    while (n <= 99999) {
      const candidate = `NCL-${String(n).padStart(3, '0')}`
      n += 1
      if (!used.has(candidate)) { used.add(candidate); return candidate }
    }
    const fallback = `NCL-${Date.now()}`
    used.add(fallback)
    return fallback
  }
}
...
if (!fs.existsSync(leadsPath)) throw new Error('Leads JSON not found. Expected Letters/prospecting/newcastle_day1_leads_100.json')
...
if (!Array.isArray(incomingRaw) || incomingRaw.length === 0) throw new Error('No rows found in Newcastle leads JSON.')
...
const nextNclId = nextNclIdFactory(existingLeads.map((l) => l.lead_id))
...
for (const lead of incomingUnique) {
  ...
  rowsToInsert.push(normalizeLeadForInsert(lead, nextNclId()))
...
const { count, error: countError } = await supabase
  .from('Leads').select('*', { count: 'exact', head: true }).like('lead_id', 'NCL-%')
if (countError) throw new Error(`NCL count check failed: ${countError.message}`)
...
console.log(`Current NCL count in Supabase: ${count}`)
