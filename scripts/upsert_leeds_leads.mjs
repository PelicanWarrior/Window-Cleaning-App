...
const leadsPath = path.join(root, 'Letters', 'prospecting', 'leeds_day1_leads_100.json')
...
function nextLdsIdFactory(existingLeadIds) {
  const used = new Set(existingLeadIds.map((id) => String(id || '').trim()).filter(Boolean))
  let n = 1
  return function nextId() {
    while (n <= 99999) {
      const candidate = `LDS-${String(n).padStart(3, '0')}`
      n += 1
      if (!used.has(candidate)) { used.add(candidate); return candidate }
    }
    const fallback = `LDS-${Date.now()}`
    used.add(fallback)
    return fallback
  }
}
...
if (!fs.existsSync(leadsPath)) throw new Error('Leads JSON not found. Expected Letters/prospecting/leeds_day1_leads_100.json')
...
if (!Array.isArray(incomingRaw) || incomingRaw.length === 0) throw new Error('No rows found in Leeds leads JSON.')

// Dedupe within the incoming file itself
...
const nextLdsId = nextLdsIdFactory(existingLeads.map((l) => l.lead_id))
...
for (const lead of incomingUnique) {
  ...
  rowsToInsert.push({ ...lead, lead_id: nextLdsId() })
...
// Post-insert duplicate cleanup pass
...
const { count, error: countError } = await supabase
  .from('Leads').select('*', { count: 'exact', head: true }).like('lead_id', 'LDS-%')
if (countError) throw new Error(`LDS count check failed: ${countError.message}`)
...
console.log(`Current LDS count in Supabase: ${count}`)
