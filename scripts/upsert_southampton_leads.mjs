...
const leadsPath = path.join(root, 'Letters', 'prospecting', 'southampton_day1_leads_100.json')
...
function nextSouIdFactory(existingLeadIds) {
  const used = new Set(existingLeadIds.map((id) => String(id || '').trim()).filter(Boolean))
  let n = 1
  return function nextId() {
    while (n <= 99999) {
      const candidate = `SOU-${String(n).padStart(3, '0')}`
      n += 1
      if (!used.has(candidate)) { used.add(candidate); return candidate }
    }
    const fallback = `SOU-${Date.now()}`
    used.add(fallback)
    return fallback
  }
}
...
async function fetchAllLeads(supabase) {
  ...
  for (let from = 0; ; from += pageSize) {
    const { data, error } = await supabase
      .from('Leads')
      .select('id, lead_id, business_name, phone, email, source_url')
...
if (!fs.existsSync(leadsPath)) throw new Error('Leads JSON not found. Expected Letters/prospecting/southampton_day1_leads_100.json')
...
if (!Array.isArray(incomingRaw) || incomingRaw.length === 0) throw new Error('No rows found in Southampton leads JSON.')
...
const nextSouId = nextSouIdFactory(existingLeads.map((l) => l.lead_id))
...
for (const lead of incomingUnique) {
  ...
  rowsToInsert.push(normalizeLeadForInsert(lead, nextSouId()))
...
const { count, error: countError } = await supabase
  .from('Leads').select('*', { count: 'exact', head: true }).like('lead_id', 'SOU-%')
if (countError) throw new Error(`SOU count check failed: ${countError.message}`)
...
console.log(`Current SOU count in Supabase: ${count}`)
