...
$locations = $locations | ForEach-Object { $_ -replace ' ', '' }
...
foreach ($url in $all) {
  ...
  $isTargetArea = [regex]::IsMatch($area, 'lanarkshire|renfrewshire|dunbartonshire|stirlingshire|west-lothian|clackmannanshire|midlothian')
...
$outFile = Join-Path $outDir 'glasgow_day1_leads_100.csv'
