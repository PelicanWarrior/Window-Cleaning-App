...
const leadsPath = path.join(root, 'Letters', 'prospecting', 'liverpool_day1_leads_100.json')
...
function nextLvpIdFactory(existingLeadIds) {
  const used = new Set(existingLeadIds.map((id) => String(id || '').trim()).filter(Boolean))
  let n = 1
  return function nextId() {
    while (n <= 99999) {
      const candidate = `LVP-${String(n).padStart(3, '0')}`
      n += 1
      if (!used.has(candidate)) { used.add(candidate); return candidate }
    }
    const fallback = `LVP-${Date.now()}`
    used.add(fallback)
    return fallback
  }
}
...
if (!fs.existsSync(leadsPath)) throw new Error('Leads JSON not found. Expected Letters/prospecting/liverpool_day1_leads_100.json')
...
if (!Array.isArray(incomingRaw) || incomingRaw.length === 0) throw new Error('No rows found in Liverpool leads JSON.')
...
const nextLvpId = nextLvpIdFactory(existingLeads.map((l) => l.lead_id))
...
for (const lead of incomingUnique) {
  ...
  rowsToInsert.push(normalizeLeadForInsert(lead, nextLvpId()))
...
const { count, error: countError } = await supabase
  .from('Leads').select('*', { count: 'exact', head: true }).like('lead_id', 'LVP-%')
if (countError) throw new Error(`LVP count check failed: ${countError.message}`)
...
console.log(`Current LVP count in Supabase: ${count}`)
